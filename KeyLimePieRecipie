...
import requests
import torch
from transformers import AutoModelForImageClassification, AutoTokenizer, AutoProcessor
import camelot
...
# Function to generate detailed descriptions for each image using CogVLM
def cogvlm_description(image_path, processor, model):
...
# CogVLM model setup
def setup_cogvlm_model():
    model_name = "THUDM/cogvlm2"  # Make sure you have the correct model path
...
# Main function to process the PDF and integrate text, tables, and CogVLM image descriptions
def process_pdf_with_cogvlm(pdf_path):
    ...
    # Step 4: Setup CogVLM model for image descriptions
    processor, model = setup_cogvlm_model()

    # Step 5: Generate detailed descriptions for each image using CogVLM
    image_descriptions = []
    for image_path in image_paths:
        description = cogvlm_description(image_path, processor, model)
    ...
    output_file = "final_output_with_text_images_tables_cogvlm.txt"
...
process_pdf_with_cogvlm(pdf_path)
